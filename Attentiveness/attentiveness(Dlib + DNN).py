...
def detect_attentiveness(eye_threshold):
    (lStart, lEnd) = face_utils.FACIAL_LANDMARKS_68_IDXS["left_eye"]
    (rStart, rEnd) = face_utils.FACIAL_LANDMARKS_68_IDXS["right_eye"]        
    ...
    frame_check = 10
    cap=cv2.VideoCapture(0)
    ear_threshold = eye_threshold - 0.04
    while True:
        ret,img = cap.read()
        img = imutils.resize(img, width=450)
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
...
if __name__ == "__main__":
    ...
    predict = dlib.shape_predictor(os.getcwd()+r"\data\shape_predictor_68_face_landmarks.dat")
    eye_threshold = calibrate()
    detect_attentiveness(eye_threshold)
